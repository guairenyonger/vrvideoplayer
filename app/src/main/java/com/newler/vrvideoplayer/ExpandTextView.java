...
/**
 * 可折叠的textview控件，点击下拉按钮可展开textview全部内容，再次点击收回。
 * @author
 * @time 2018/3/21
 */
public class ExpandTextView extends LinearLayout implements View.OnClickListener {
    ...
    public ExpandTextView(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        titleTextSize = a.getDimension(R.styleable.ExpandTextView_titleTextSize, DensityUtil.dip2px(mContext, DEFAULT_TITLE_TEXT_SIZE));
        contentTextSize = a.getDimension(R.styleable.ExpandTextView_contentTextSize, DensityUtil.dip2px(mContext, DEFAULT_CONTENT_TEXT_SIZE));
        hintTextSize = a.getDimension(R.styleable.ExpandTextView_hintTextSize, DensityUtil.dip2px(mContext, DEFAULT_HINT_TEXT_SIZE));
    ...
    private void init() {
        ...
        if (indicateImage == null) {
            indicateImage = ContextCompat.getDrawable(mContext, R.mipmap.white_arrow_down);
        }
        ...
        post(new Runnable() {
            ...
            public void run() {
                ...
                if (mContentView.getLineCount() <= minVisibleLines) {
                    layoutParams.height = mContentView.getLineHeight() * mContentView.getLineCount();
                } else {
                    layoutParams.height = minVisibleLines * mContentView.getLineHeight();
                }
    ...
    public void setTitleTextSize(float titleTextSize) {
        this.titleTextSize = DensityUtil.dip2px(mContext, titleTextSize);
    ...
    public void setContentTextSize(float contentTextSize) {
        this.contentTextSize = DensityUtil.dip2px(mContext, contentTextSize);
    ...
    public void setHintTextSize(float hintTextSize) {
        this.hintTextSize = DensityUtil.dip2px(mContext, hintTextSize);
    ...
    public int getMaxMeasureHeight() {
        ...
        if (minVisibleLines > mContentView.getLineCount() ) {
            heightMeasureSpec = MeasureSpec.makeMeasureSpec(minVisibleLines * mContentView.getLineHeight(), MeasureSpec.AT_MOST);
        } else {
            heightMeasureSpec = MeasureSpec.makeMeasureSpec(mContentView.getLineCount() * mContentView.getLineHeight(), MeasureSpec.AT_MOST);
        }
    ...
    public int getMinMeasureHeight() {
        ...
        if (copy == null) {
            ...
            copy.setLineSpacing(DensityUtil.dip2px(mContext, 9), 1.0f);
        ...
        if (minVisibleLines <= mContentView.getLineCount() ) {
            ...
            mHintView.setTextSize(TypedValue.COMPLEX_UNIT_PX);
            heightMeasureSpec = MeasureSpec.makeMeasureSpec(mContentView.getLineCount() * mContentView.getLineHeight(), MeasureSpec.AT_MOST);
        }
